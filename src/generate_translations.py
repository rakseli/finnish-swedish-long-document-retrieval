...
def create_dataloader(args,prompt):
    ...
    if args.test:
        ...
        batch_size = 100
    dataloader = DataLoader(dataset,batch_size=batch_size,collate_fn=naive_data_collator,shuffle=False,num_workers=num_workers)
    return dataloader

def format_data(example,tokenizer,prompt):
    user = {"role": "user", "content":prompt.format(text=example['generated_text'])}
    system = {"role":"system","content":generic_system_prompt}
    example["translation_prompt"] = tokenizer.apply_chat_template([system,user],tokenize=False,add_generation_prompt=True,enable_thinking=False)
    return example
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path",default="/scratch/project_2018556/finnish-swedish-long-document-retrieval/data/swe_Latn/swe_Latn_test_dev_long_doc_subset_annotated_splitted_paragraphs_Qwen3.5-122B-A10B-GPTQ-Int4_generate_questions.jsonl",type=str,help="path to source file")
...
>>>
    ...
    out_path = os.path.join(base_dir,f"{base_name}_translations.jsonl")
    batches_to_skip = 0
    ...
    with open(out_path, file_mode, encoding="utf-8") as fi:
        ...
        for batch_index,batch in enumerate(dl_iter,start=1):
            ...
            if elapsed_time > args.exit_duration_in_mins:
                ...
                all_generated = False
                break
            texts = [text_i['translation_prompt'] for text_i in batch]
            original_questions = [text_i['generated_text'] for text_i in batch]
            ...
            for output,text_id,text,original_question in zip(outputs,ids,texts,original_questions):
                generated_text = output.outputs[0].text
                d = {"original_question":original_question,"tranlation_prompt":text,"translated_question":generated_text,'id':text_id}
